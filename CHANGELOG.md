...
- **Import Error in README.md**: Fixed incorrect import `instaharvest.comments_export` → `instaharvest.exporters` in Example 5.
- **playwright-stealth Warning**: Fixed `'module' object is not callable` error by correctly instantiating the `Stealth` class.
- **Locale Support**: Added `locale` configuration option to `ScraperConfig` (default: `en-US`) for consistent English UI selectors.
- **Headless Mode Retry**: Implemented retry logic for browser launch to handle 'Old Headless mode' deprecation.

### Changed

- **Example 5 Cleanup**: Removed unnecessary `sys.path.insert` and added proper `ScraperConfig` usage in README.md.
